...
from .resnet.sppad import Conv2d as spConv2d
...
class _Upsample(nn.Module):
    def __init__(self, num_maps_in, skip_maps_in, num_maps_out, use_bn=True, k=3):
        ...
        self.se = SE(num_maps_in)

    def forward(self, x, skip):
        skip = self.bottleneck.forward(skip)
        skip = self.se(skip)  # add channel attention
...
class SpatialPyramidPooling(nn.Module):
    def __init__(
        ...
        self.spp.add_module(
            "spp_bn", _BNReluConv(num_maps_in, bt_size, k=1, bn_momentum=bn_momentum, batch_norm=use_bn)
        )
        ...
        for i in range(num_levels):
            ...
            self.spp.add_module(
                "spp" + str(i), _BNReluConv(num_features, level_size, k=1, bn_momentum=bn_momentum, batch_norm=use_bn)
            )
        self.spp.add_module(
