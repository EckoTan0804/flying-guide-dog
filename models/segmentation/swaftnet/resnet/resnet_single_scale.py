...
from .sppad import Conv2d as spConv2d
...
def conv3x3(in_planes, out_planes, stride=1, use_sppad=False, split=1):
    """3x3 convolution with padding"""
    return spConv2d(
        in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False, use_sppad=use_sppad, split=split
    )
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, downsample=None, efficient=True, use_bn=True, use_sppad=False, split=1
    ):
        super(BasicBlock, self).__init__()
        self.use_bn = use_bn
        self.conv1 = conv3x3(inplanes, planes, stride, use_sppad=use_sppad, split=split)
        self.bn1 = nn.BatchNorm2d(planes) if self.use_bn else None
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes, use_sppad=use_sppad, split=split)
...
class ResNet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.efficient = efficient
        self.use_bn = use_bn
        self.conv1 = spConv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False, use_sppad=use_sppad, split=split)
    ...
    def forward_down(self, image):
        ...
        features += [F.max_pool2d(skip, (1, self.split), (1, self.split))]
        ...
        features += [self.spp.forward(F.max_pool2d(skip, (1, self.split), (1, self.split)))]
        return features
...
def resnet18(pretrained=True, use_sppad=False, split=1, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], use_sppad=use_sppad, split=split, **kwargs)
    if pretrained:
